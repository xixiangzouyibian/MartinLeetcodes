...
class Solution {
    ...
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        ...
        dfs(sr, sc, row, col, image, targetColor, newColor, visited);
        return image;
    }

    private void dfs(int x, int y, int row, int col, int[][] image, int targetColor, int newColor, boolean[][] visited) {
        if (!visited[x][y]) {
            visited[x][y] = true;
            image[x][y] = newColor;
            for (int[] d : directions) {
                ...
                if (X >= 0 && X < row && Y >= 0 && Y < col && image[X][Y] == targetColor) {
                    dfs(X, Y, row, col, image, targetColor, newColor , visited);
                }
            }
        }
    }
