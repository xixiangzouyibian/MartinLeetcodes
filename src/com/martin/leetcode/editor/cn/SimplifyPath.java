...
class Solution {
    public String simplifyPath(String path) {
        ...
        for (char c : path.toCharArray()) {
            if (c == '/') {
                if (!stack.isEmpty()) {
                    String preLevel = stack.peekLast().toString();
                    if (preLevel.equals("/.") || preLevel.equals("/")) {
                        stack.pollLast();
                    } else if (preLevel.equals("/..")) {
                        for (int j = 0; j < 2 && !stack.isEmpty(); j++) {
                            stack.pollLast();
                        }
                    }
                }
                stack.offerLast(new StringBuilder("/"));
            } else {
                stack.peekLast().append(c);
            }
        }
        if (stack.size() > 1) {
            stack.pollLast();
