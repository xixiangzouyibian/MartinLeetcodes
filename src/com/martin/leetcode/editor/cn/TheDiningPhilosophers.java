...
class DiningPhilosophers {
    ...
    AtomicInteger forks = new AtomicInteger(0); // 00000
    int[] forkMask = new int[] {1, 2, 4, 8, 16}; // 00001, 00010, 00100, 01000, 10000
    ...
    public void wantsToEat(int philosopher,
        ...
        pickBoth.lock();
        while (!acquireFork(forkMask[philosopher])) Thread.sleep(1);
        while (!acquireFork(forkMask[(philosopher+1) % 5])) Thread.sleep(1);
        pickLeftFork.run();
        pickRightFork.run();
        pickBoth.unlock();
        ...
        while (!releaseFork(forkMask[philosopher])) Thread.sleep(1);
        while (!releaseFork(forkMask[(philosopher+1) % 5])) Thread.sleep(1);
    }

    private boolean acquireFork(int mask) {
        int expect = forks.get();
        return (expect & mask) == 0 && forks.compareAndSet(expect, expect ^ mask);
    }

    private boolean releaseFork(int mask) {
        int expect = forks.get();
        return forks.compareAndSet(expect, expect ^ mask);
    }
}
