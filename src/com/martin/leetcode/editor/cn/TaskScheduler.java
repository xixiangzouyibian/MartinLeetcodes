...
/*    public int leastInterval(char[] tasks, int n) {
    ...
    public int leastInterval(char[] tasks, int n) {
        ...
        Arrays.sort(map);
        int bucket_num = map[25], bucket_capacity = n+1;
        int least_interval = (bucket_num-1) * bucket_capacity + 1;
        int i = 24;
        while (i >= 0 && map[i--] == bucket_num) {
            least_interval++;
        }
        return Math.max(least_interval, tasks.length);
    }
}
